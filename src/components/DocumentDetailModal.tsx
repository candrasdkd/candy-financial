...
import { Trash2, Copy, Check, X, ScanLine, Shield, Download, Share2, Loader2, Pencil, Save, ChevronLeft, ChevronRight, Sparkles } from 'lucide-react';
      ...
      >
            ...
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                {isEditing ? (
                  ...
                  <h2 className="font-display text-xl text-sage-900 leading-tight truncate">{doc.name}</h2>
                )}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-hide">
          ...
          <div className="relative group/gallery">
            <div ref={scrollRef} onScroll={onScroll} className="flex overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-2 scroll-smooth">
              {(doc.imageUrls || [doc.imageUrl!]).map((url, i) => (
                <div key={i} className="flex-shrink-0 w-full snap-center px-1">
                  <img src={url} alt={`${doc.name} ${i+1}`} className="w-full h-auto max-h-[40vh] object-contain rounded-3xl bg-sage-50 border border-sage-100 shadow-sm" />
                </div>
              ))}
            </div>
          ...
          <div className="space-y-4">
            <label className="text-[9px] font-bold text-sage-400 uppercase tracking-widest px-1 flex items-center gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isEditing ? (
                        <input type="text" value={field.value} onChange={e => {
                          const n = [...editedFields]; n[i].value = e.target.value; setEditedFields(n);
